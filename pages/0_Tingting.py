...
left, right = st.columns(2)
with left:
    n_months = st.slider('Number of months', 1, 36, 1)
with right:
    CHV_L4_b = st.slider('Influence of CHV on increasing probability of L4', 0.00, 0.10, 0.01)
    #submitted = st.form_submit_button("Run")
...
Capacity_ratio1_df = pd.DataFrame(Capacity_ratio1)
Capacity_ratio1_df['Month'] = np.arange(n_months)
Capacity_ratio1_df_melt = pd.melt(Capacity_ratio1_df, id_vars=['Month'])
Capacity_ratio1_df_melt = Capacity_ratio1_df_melt.rename(columns={'variable': 'Subcounty'})
Capacity_ratio1_df_melt['Subcounty'] = Capacity_ratio1_df_melt['Subcounty'].astype('category')

chart_Capacity_ratio1 = (
        alt.Chart(
            data=Capacity_ratio1_df_melt,
            title="Capacity_ratio1",
        )
        .mark_line()
        .encode(
...
with left:
    #if submitted:
    st.altair_chart(chart_Capacity_ratio1)

Capacity_ratio2_df = pd.DataFrame(Capacity_ratio2)
Capacity_ratio2_df['Month'] = np.arange(n_months)
Capacity_ratio2_df_melt = pd.melt(Capacity_ratio2_df, id_vars=['Month'])
Capacity_ratio2_df_melt = Capacity_ratio2_df_melt.rename(columns={'variable': 'Subcounty'})
Capacity_ratio2_df_melt['Subcounty'] = Capacity_ratio2_df_melt['Subcounty'].astype('category')

chart_Capacity_ratio2 = (
        alt.Chart(
            data=Capacity_ratio2_df_melt,
            title="Capacity_ratio2",
        )
        .mark_line()
        .encode(
            x=alt.X("Month", axis=alt.Axis(title="Month")),
...
with right:
    #if submitted:
    st.altair_chart(chart_Capacity_ratio2)
