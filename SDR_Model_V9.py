...
with st.sidebar:
    ...
    imple_time = st.slider("The length of implementation phase (years)?",  min_value=1, max_value=3, step=1, value=3,
                          help = 'Implementaion phase will stop at the years you choose \n\n and continue the maintainance phase')
    int_period = imple_time * 12

    main_time = st.slider("The length of maintenance phase (years)?",  min_value=1, max_value=7, step=1, value=3,
                          help = 'The model will simulate until maintenance phase finish')

    n_months = main_time * 12 + int_period
...
with col1:
    ...
    with col1_1:
        ...
        CHVint = st.checkbox('Employ CHVs', help = "CHVs refer to Community Healthcare Workers")
        if CHVint:
            flag_sdr = 1
            flag_CHV = 1
            CHV_pushback = True
            flag_ANC = 1
            demand_scenarios = st.selectbox('Select demand scenarios:', ['Conservative', 'Moderate', 'Aggressive'])
    with col1_2:
        st.text('Adjust parameters')
        if CHVint and demand_scenarios == 'Conservative':
            Exp_45_add = st.slider("% increase in **L4/5 deliveries**", min_value=0.00, max_value=0.20, step=0.05, value=0.20,
                               help = "Value = 0.2 means L4/5 deliveries will increase by 20% \n\n by the end of implementation phase and keep the same in the maintainance phase")

            ANCadded = st.slider('% increase in **4+ANC visits**', min_value=0.00, max_value=0.05, step=0.01, value=0.05,
                                 help = "Value = 0.05 means the ANC rate will increase by 5% \n\n by the end of implementation phase and keep the same in the maintainance phase")

        if CHVint and demand_scenarios == 'Moderate':
            Exp_45_add = st.slider("% delivery in L4/5 facilities will increase by (20% to 50%)", min_value=0.20, max_value=0.50, step=0.05, value=0.50,
                               help = "Value = 0.5 means L4/5 deliveries will increase by 50% \n\n by the end of implementation phase and keep the same in the maintainance phase")

            ANCadded = st.slider('4+ANC rate will increase by (5% to 15%)', min_value=0.05, max_value=0.15, step=0.05, value=0.15,
                                 help = "Value = 0.2 means the ANC rate will increase by 15% \n\n by the end of implementation phase and keep the same in the maintainance phase")

        if CHVint and demand_scenarios == 'Aggressive':
            Exp_45_add = st.slider("% delivery in L4/5 facilities will increase by (50% to 100%)", min_value=0.50, max_value=1.00, step=0.10, value=1.00,
                               help = "Value = 1.0 means L4/5 deliveries will increase by 100% \n\n by the end of implementation phase and keep the same in the maintainance phase")

            ANCadded = st.slider('4+ANC rate will increase by (15% to 20%)', min_value=0.15, max_value=0.20, step=0.05, value=0.20,
                                 help = "Value = 0.2 means the ANC rate will increase by 20% \n\n by the end of implementation phase and keep the same in the maintainance phase")
...
with col2:
    ...
    with col2_1:
        ...
        if facint:
            ...
            supply_scenarios = st.selectbox('Select supply scenarios:', ['Conservative', 'Moderate', 'Aggressive'])
    with col2_2:
        st.text('Adjust parameters')
        if facint and supply_scenarios == 'Conservative':
            know_added = st.slider("Improve knowledge of healthcare workers by (0% to 20%)", min_value=0.00, max_value=0.20, step=0.05, value=0.20,
                                   help = "Knowledge reflects the likelihood of following protocols of single interventions \n\n"
                                          "Value = 0.2 means to increase knowledge by 20% \n\n by the end of implementation phase and keep the same in the maintainance phase")

            improve_capacity = st.checkbox("Improve capacity by upgrading infrastructures with **only high priority**")
            if improve_capacity:
                capacity_added = 0.2

        if facint and supply_scenarios == 'Moderate':
            know_added = st.slider("Improve knowledge of healthcare workers by (20% to 50%)", min_value=0.20,
                                   max_value=0.50, step=0.05, value=0.50,
                                   help="Knowledge reflects the likelihood of following protocols of single interventions \n\n"
                                        "Value = 0.5 means to increase knowledge by 50% \n\n by the end of implementation phase and keep the same in the maintainance phase")

            improve_capacity = st.checkbox("Improve capacity by upgrading infrastructures with **both high and :blue[medium priority]**")
            if improve_capacity:
                capacity_added = 0.5

        if facint and supply_scenarios == 'Aggressive':
            know_added = st.slider("Improve knowledge of healthcare workers by (50% to 100%)", min_value=0.50,
                                   max_value=1.00, step=0.05, value=1.00,
                                   help="Knowledge reflects the likelihood of following protocols of single interventions \n\n"
                                        "Value = 1.0 means to increase knowledge by 100% \n\n by the end of implementation phase and keep the same in the maintainance phase")

            improve_capacity = st.checkbox("Improve capacity by upgrading infrastructures with **all high, medium, and low priority**")
            if improve_capacity:
                capacity_added = 1.0

            #capacity_added = st.slider("Improve facility capacity)", min_value=0.0, max_value=1.0, step=0.1, value=0.5,
    ...
    #
    #    if refint:
    #        rescue_scenarios = st.selectbox('Select rescue scenarios:', ['Conservative', 'Moderate', 'Aggressive'])
    #with col2_4:
    #    if refint:
    #        flag_sdr = 1
    #
...
with col3:
    ...
    with col3_10:
        if intus:
            flag_us = 1
            ultrasound_effectiveness = st.slider("Effectiveness of ultrasound", min_value=0.0, max_value=1.00, step=0.1, value=0.10,
                              help = "It reflects the increased likelihood of mothers' with complications can be referred to L4/5 from homes/L23\n\n"
                                     "When value = 1, the complications transferred to L4/5 increased by 1.5 times")
st.markdown("~~~")
...
with ((st.form('Test'))):
    ...
    def set_INT(param, i, j, subcounty, f_intvs, i_scenarios, sc, comps, global_vars, int_period):
        ...
        #calculate actual capacity added
        current_volumn = np.array([2245, 1579, 1288, 2045, 2248,
                                   3907 + 118 + 91 + 77 + 99 + 134 + 169 + 122 + 81 + 91 + 114 + 124,
                                   3349, 1326, 868, 2258, 1579, 1851])

        base_capacity = np.array(
            [3000.0, 1992.0, 1608.0, 2328.0, 2784.0, 6060.0, 4176.0, 2736.0, 1308.0, 5820.0, 2088.0, 5124.0])

        exp_capacity_ratio = current_volumn * (1 + capacity_added) / base_capacity
        act_capacity_added = np.where(exp_capacity_ratio < 1, 0, exp_capacity_ratio - 1)

        # usage
        if subcounty[j] == 1:
            if flag_sdr:
                temp_knowadded = min(1 + know_added / int_period * i, 1 + know_added)
                int_knowl = min(1, knowl * temp_knowadded)
            ...
            if flag_CHV:
                INT['CHV']['sdr_effect'][j] = Exp_45_add / int_period
                flag_ANC = 1
            else:
                INT['CHV']['sdr_effect'][j] = 0
            if flag_ANC:
                INT['ANC']['effect'][j] = min(1 + ANCadded / int_period * i, 1 + ANCadded)
            ...
            if flag_capacity:
                tem_capacity_added = min(1 + act_capacity_added[j] / int_period * i, 1 + act_capacity_added[j])
                #Capacity[j] = Capacity[j] * tem_capacity_added
                Capacity[j] = base_capacity[j] / 12 * tem_capacity_added
            else:
            ...
            if flag_refer_capacity:
                temp_ref_cap_added = min(1 + global_vars['refer_capacity_added'] / int_period * i, 1 + global_vars['refer_capacity_added'])
                INT['SDR']['refer_capacity'][j] = 100 * temp_ref_cap_added
            else:
    ...
    def run_model(param, subcounty, f_intvs, t, n_months, global_vars, int_period):
        ...
        for i in t:
            ...
            for j in range(sc_time['n']):
                ...
                if i > 0:
                    ...
                    f_comps_new = SC_LB / original
                    comps_refer = f_comps_new * sc_time['LB1s'][j][i, :] - f_comps * sc_time['LB1s'][j][i, :]

                    anc = min(1, sc['ANC'][j] * i_INT['ANC']['effect'][j])
                    comps_overall = np.sum((f_comps * sc_time['LB1s'][j][i, :])) / np.sum(sc_time['LB1s'][j][i, :])
                    f_refer = comps_refer / sc_time['LB1s'][j][i, :]
                ...
                #st.text(np.sum(sc_time['LB1s'][11][0, 2:3])*12)
                if j != 5:
                    ...
                    Capacity_ratio[i, j] = sum(sum(sc_time['LB1s'][j][i, 3:4]) for j in sc_time['LB1s']) / Capacity[j]
                    opinion[i, j] = sum(sum(sc_time['LB1s'][j][i, 3:4]) for j in sc_time['LB1s']) / np.sum(sc_time['LB1s'][j][i, list([0, 1, 3])])
    ...
    def f_LB_effect(SC_LB_base, Com_ref, SC_LB_previous, INT, flag_pushback, j, i, Capacity_ratio, opinion, Capacity):
        ...
        INT_b = INT['CHV']['sdr_effect'][j] if i < int_period else 0
    ...
    if submitted:
        ...
        b_outcomes = run_model(param, [0]*12, b_f_intvs, t, n_months, b_global_vars, int_period) #run_model([], b_flags)
        #b_outcomes
        outcomes = run_model(param, SDR_subcounties, f_intvs, t, n_months, global_vars, int_period) #run_model(SCID_selected, flags)
        ...
        if selected_plotA == "Cost effectiveness":
            ...
            b_df = run_model(param, [0]*12, b_f_intvs, t, n_months, global_vars, int_period)
            ...
            if ce_int1:
                ...
                df = run_model(param, SDR_subcounties, f_intvs, t, n_months, global_vars, int_period)
            ...
            if ce_int2:
                ...
                df_aggregate = get_aggregate(df)
                df_years = agg_years(df_aggregate, n_months)
                op_int2 = get_cost_effectiveness(INTs, time_comp[1], df_years, b_df_years, global_vars, int_period)
            else:
                op_int2 = [0] * 18
            ...
            if ce_int6:
                ...
                df_aggregate = get_aggregate(df)
                df_years = agg_years(df_aggregate, n_months)
                op_int6 = get_cost_effectiveness(INTs, time_comp[1], df_years, b_df_years, global_vars, int_period)
            else:
                op_int6 = [0] * 18
            ...
            if ce_sdr2:
                ...
                df_aggregate = get_aggregate(df)
                df_years = agg_years(df_aggregate, n_months)
                op_sdr2 = get_cost_effectiveness(INTs, time_comp[1], df_years, b_df_years, global_vars, int_period)
            else:
                op_sdr2 = [0] * 18
            ...
            if ce_sdr3:
                ...
                df_aggregate = get_aggregate(df)
                df_years = agg_years(df_aggregate, n_months)
                op_sdr3 = get_cost_effectiveness(INTs, time_comp[1], df_years, b_df_years, global_vars, int_period)
            else:
                op_sdr3 = [0] * 18
            ...
            if ce_sdr4:
                ...
                df_aggregate = get_aggregate(df)
                df_years = agg_years(df_aggregate, n_months)
                op_sdr4 = get_cost_effectiveness(INTs, time_comp[1], df_years, b_df_years, global_vars, int_period)
            else:
                op_sdr4 = [0] * 18
        ...
        if selected_plotA == "Referral from home to L45":
            ...
            ymax = max(dfs[0].iloc[:, 2].max(), dfs[1].iloc[:, 2].max())
